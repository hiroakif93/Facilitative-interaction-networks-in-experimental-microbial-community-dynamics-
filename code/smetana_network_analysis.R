...
# -- Load data table
crosfeed <- read.table('table/smetana_result.tsv', header=T)
crosfeed <- apply(crosfeed, 2, function(x){ gsub('_rename', '', x) } )

mag <- readRDS('table/MAG_infomation.rds') 
mag[mag ==''] <- 'Unidentified'
repmag <- mag[unique(mag$represent),]

ts <- readRDS('table/MAG_timeSeries.rds')
ts <- ts/rowSums(ts)

cols <- readRDS('table/color_palette.rds')
...
g <- ggplot(cent2)+
    geom_point(aes(x= (influence.weght), y= (pagerank.weght), fill=Genus, shape=Class), size=4)+
...
for(i in as.character( unique(crosfeed[,1]) ) ){ #i=as.character( unique(crosfeed[,1]) )[1]
    ...
    gnet <- ggnetwork(graph, arrow.gap=0.05)
